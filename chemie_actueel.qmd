...
```{r}
reactableOutput("tabel_metingen")
```
...
output$tabel_metingen <- renderReactable({
  
  reactable_agg <- ifelse(input$agg_fun %in% c("mean", "median", "min", "max"), input$agg_fun, "mean")
  
  waarde_agg <- 
  c("max" = "maximum",
  "min" = "minimum",
  "median" = "mediaan",
  "mean" = "gemiddelde")[reactable_agg]
  
  waarde_agg <- glue("Meetwaarde {f_eenheid(input$param_sel)} ({waarde_agg})")
  
  tabel_metingen <- 
    fys_chem_sel() |> 
    # mutate("Aantal metingen" = n(), .by = "mp") |> 
    mutate("Parameter" = f_parnaam(parnr)) |> 
    
    select(mp, Parameter, Datum = datum, Detectiegrens = detectiegrens, waarde) |> 
    reactable(
          filterable = TRUE, 
          searchable = TRUE,
          onClick = "expand", 
          ...
          columns = list(
            mp = colDef("Meetpunt"),
            waarde = colDef(waarde_agg, aggregate = reactable_agg, filterable = FALSE, format = colFormat(digits = 2))
