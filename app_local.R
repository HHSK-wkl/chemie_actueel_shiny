...
server <- function(input, output) {
  ...
  meetpunten <- HHSKwkl::import_meetpunten("data/meetpunten.csv")
  parameters <- read.csv2("data/parameters.csv")
  fys_chem <- readRDS("data/fys_chem.rds") %>% semi_join(filter(meetpunten, meetpunttypering %in% c(1, 2, 3, 5, 12)))
